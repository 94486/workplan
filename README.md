...
> 科技感界面（星点闪烁 · 光束扫过 · 透视网格背景） · 零前端依赖（原生 JS + SVG 图表）· 前后端分离结构 · 数据实时同步
...
| 📝 工作录入 | 支持两类工作：**常规工作** / **其他工作**；字段：工作名称、花费时长、计划完成日期、预期收入、备注；点击空白格快速录入；**录入记忆按类型独立**——常规/其他各自记忆上次填写内容，互不影响，切换类型时自动载入对应记忆 |
...
| `PORT` | `8000` | 服务监听端口 |
| `HOST` | Docker/源码 `0.0.0.0`，exe `127.0.0.1` | 监听地址；exe 默认仅本机访问，需要局域网访问时设为 `0.0.0.0` |
