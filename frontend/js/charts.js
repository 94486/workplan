...
const Charts = (() => {
  ...
  function comboChart(svg, {
    ...
    const yMaxR = niceMax(maxBar * 1.15);
    const yMaxL = niceMax(maxLine * 1.15);

    const barGrad = vGradient(svg, "barGrad" + uid, barSeries.color, 0.15, 0.92);
    const lineGrad = vGradient(svg, "lineGrad" + uid, lineSeries.color, 0.30, 0.02);
    const barGlow = glowFilter(svg, "barGlow" + uid, barSeries.color, 2.5);
    const lineGlow = glowFilter(svg, "lineGlow" + uid, lineSeries.color, 3);
    uid++;
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      el("line", {
        x1: left, x2: right, y1: y, y2: y, class: "axis-line",
        "stroke-width": 0.6, "stroke-dasharray": i === steps ? "none" : "3 5",
      }, svg);
      el("text", { x: left - 8, y: y + 4, "text-anchor": "end", class: "tick-label" }, svg)
        .textContent = fmtL(valL);
      el("text", { x: right + 8, y: y + 4, "text-anchor": "start", class: "tick-label" }, svg)
        .textContent = fmtR(valR);
    }
    // X 轴基线
    ...
    barSeries.values.forEach((v, i) => {
      ...
      el("rect", { x, y, width: barW, height: 2.2, rx: 1, fill: barSeries.color, opacity: 0.98 }, svg);
      el("text", {
        x: cx(i), y: y - 5, "text-anchor": "middle",
        "font-size": 10, fill: barSeries.color,
      }, svg).textContent = fmt(v);
    });

    // 折线（收入，左轴）：平滑曲线 + 面积渐变 + 发光节点
    ...
    if (pts.length) {
      ...
      pts.forEach((p, i) => {
        ...
        if (v <= 0) return;
        el("circle", { cx: p.x, cy: p.y, r: 3.6, fill: "var(--bg-1)", stroke: lineSeries.color, "stroke-width": 2, filter: `url(#${lineGlow})` }, svg);
        el("text", { x: p.x, y: p.y - 9, "text-anchor": "middle", "font-size": 10, fill: lineSeries.color, "font-weight": 700 }, svg)
          .textContent = fmt(v);
      });
    }
    ...
    // X 轴标签
    labels.forEach((lb, i) => {
      el("text", { x: cx(i), y: bottom + 20, "text-anchor": "middle", "font-size": 11 }, svg)
        .textContent = lb;
    });

    // 图例
    let lx = left;
    [[barSeries, "rect"], [lineSeries, "line"]].forEach(([s, kind]) => {
      if (kind === "rect") {
        el("rect", { x: lx, y: 8, width: 10, height: 10, rx: 2, fill: s.color }, svg);
      } else {
        el("line", { x1: lx, x2: lx + 12, y1: 13, y2: 13, stroke: s.color, "stroke-width": 2.4, "stroke-linecap": "round" }, svg);
        el("circle", { cx: lx + 6, cy: 13, r: 2.6, fill: "var(--bg-1)", stroke: s.color, "stroke-width": 1.6 }, svg);
      }
  ...
  function drawAxes(svg, { left, top, right, bottom, yMax, xCount, fmtY }) {
    ...
    el("stop", { offset: "0%", "stop-color": "rgba(56,189,248,.25)" }, grad);
    el("stop", { offset: "100%", "stop-color": "rgba(56,189,248,.95)" }, grad);
